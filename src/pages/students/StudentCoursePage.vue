...
const searchCourse = async (req, res) => {
  try {
    const response = await axios.get(
      `http://localhost:3000/api/courses/search/${user.value._id}?courseQuery=${searchQuery.value}`,
    );
  ...
  <div class="container mx-auto p-4 flex justify-center items-center">
    <div class="flex w-full max-w-lg items-center gap-4">
      ...
      <div class="relative flex-grow">
        ...
        <i
          @click="searchCourse"
          class="hover:text-green-500 cursor-pointer fa-solid fa-magnifying-glass text-gray-400 absolute right-4 top-1/2 -translate-y-1/2"
        ></i>
      </div>
    </div>
  </div>
